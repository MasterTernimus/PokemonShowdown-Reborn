	...
	*/
		...
		default:
			...
			if (this.battle.activePerHalf > 1 && !move.tracksTarget) {
				const isCharging = move.flags['charge'] && !this.volatiles['twoturnmove'] &&
					!(move.id === 'dive' && this.battle.field.isTerrain([
						'watersurfaceterrain', 'underwaterterrain', 'midnightzoneterrain',
					])) &&
	...
	pollenbloom: ['proficient', 'thickfat'],
		...
		sweetsanctuary: ['friendguard', 'sweetveil', 'aromaveil', 'pastelveil'],
			...
			absolutezero: ['moldbreaker', 'filter'],
			...
			burningego: ['ultraego', 'magmaarmor', 'proficient'],
			burningspirit: ['selfsufficient', 'opportunist', 'magmaarmor', 'filter', 'proficient'],
			crueltag: ['shadowtag', 'infiltrator'],
			emperorsresolve: ['competitive', 'slushrush', 'proficient'],
			execution: ['duskilate'],
			fallenstar: ['moldbreaker', 'dualwield', 'skilllink', 'selfsufficient', 'proficient'],
			parasitism: ['dryskin'],
		...
		wickedsnare: ['stakeout', 'tanglinghair', 'prankster'],
			...
			astralwitchcraft: ['levitate', 'magicguard', 'swornduty', 'proficient'],
			ragingcurrent: ['swiftswim', 'damp', 'waterveil', 'dryskin', 'stamina', 'proficient'],
			calderacore: ['magmaarmor', 'sheerforce', 'drought', 'solidrock'],
		...
		heatcoil: ['speedboost', 'magmaarmor', 'flamebody'],
			ironcognition: ['toughclaws', 'prismarmor'],
			ironwill: ['prismarmor', 'secondwind', 'selfsufficient'],
			joyride: ['aerilate', 'hypercutter', 'vitalspirit'],
...
riotamp: ['galvanize', 'resonanceforce', 'voltabsorb'],
	...
	effectiveWeather(message?: string | boolean) {
		...
		if (this.hasAbility(['megasol', 'bloomingsun']) && this.battle.activePokemon === this && weather !== 'sunnyday') {
			if (message) this.battle.add('-activate', this, 'ability: ' + this.getAbility().name);
