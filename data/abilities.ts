...
const WEIGHT_BASE_POWER_MOVES = new Set(['grassknot', 'heavyslam', 'heatcrash', 'lowkick']);
...
const Z_PROTEAN_FORMES: Record<string, { id: number, species: string }> = {
	...
	Flying: { id: 2, species: 'Nimbeon' },
	Poison: { id: 3, species: 'Toxeon' },
	Ground: { id: 4, species: 'Dusteon' },
	Rock: { id: 5, species: 'Basaleon' },
	Bug: { id: 6, species: 'Ephemeon' },
	Ghost: { id: 7, species: 'Kitsuneon' },
	Steel: { id: 8, species: 'Titaneon' },
	'???': { id: 9, species: 'Byteon' },
	Fire: { id: 10, species: 'Flareon' },
	Water: { id: 11, species: 'Vaporeon' },
	Grass: { id: 12, species: 'Leafeon' },
	Electric: { id: 13, species: 'Jolteon' },
	Psychic: { id: 14, species: 'Espeon' },
	Ice: { id: 15, species: 'Glaceon' },
	Dragon: { id: 16, species: 'Drekeon' },
	Dark: { id: 17, species: 'Umbreon' },
	Fairy: { id: 18, species: 'Sylveon' },
};

type AbilityChipType = 'Fire' | 'Grass' | 'Ground' | 'Water' | 'Rock' | 'Ice' | 'Flying';

const ABILITY_CHIP_IMMUNITIES: Partial<Record<AbilityChipType, string[]>> = {
...
function isImmuneToAbilityChip(target: Pokemon, type: AbilityChipType) {
...
function abilityChipDamage(battle: Battle, target: Pokemon, type: AbilityChipType, sixteenths = 1) {
	const typeMod = battle.gameType === 'freeforall' ? battle.dex.getEffectiveness(type, target) : 0;
	return battle.clampIntRange(target.baseMaxhp * sixteenths / 16 * (2 ** typeMod), 1);
}

function isStarterEeveePokemon(pokemon: Pokemon) {
	const originalSpecies = typeof pokemon.set.species === 'string' ?
		pokemon.set.species.toLowerCase().replace(/[^a-z0-9]/g, '') : '';
	return ['eeveestarter', 'eeveestarteralt'].includes(originalSpecies) ||
		['eeveestarter', 'eeveestarteralt'].includes(pokemon.baseSpecies.id);
}

function applyZProteanForm(battle: Battle, source: Pokemon, move: ActiveMove, effect: Effect) {
	if (!isStarterEeveePokemon(source) || move.id === 'struggle' || move.hasBounced ||
		move.flags['futuremove'] || move.sourceEffect === 'snatch' || move.callsMove) return;
	const type = move.type;
	const forme = Z_PROTEAN_FORMES[type];
	if (!forme) return;
	if (source.getTypes().join() !== type) {
		if (!source.setType(type)) return;
		battle.add('-start', source, 'typechange', type, '[from] ability: Z Protean');
	}
	if (source.zProteanForme === forme.id && source.zProteanVisualSpecies === forme.species) return;
	source.zProteanForme = forme.id;
	source.zProteanVisualSpecies = forme.species;
	battle.add('-formechange', source, forme.species, '[from] ability: Z Protean');
}
...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	permafrost: {
		onStart(pokemon) {
			return this.dex.abilities.get('icescales').onStart?.call(this, pokemon);
		},
		onDamagingHit(damage, target, source, move) {
		...
		onBasePower(basePower, pokemon, target, move) {
			return this.dex.abilities.get('refrigerate').onBasePower?.call(this, basePower, pokemon, target, move);
		},
		flags: { breakable: 1 },
		name: "Permafrost",
		rating: 5,
	...
	glacialheart: {
		...
		onDamagingHit(damage, target, source, move) {
			this.dex.abilities.get('thermalexchange').onDamagingHit?.call(this, damage, target, source, move);
			return this.dex.abilities.get('icebody').onDamagingHit?.call(this, damage, target, source, move);
		},
		onUpdate(pokemon) {
			return this.dex.abilities.get('thermalexchange').onUpdate?.call(this, pokemon);
		},
		onSetStatus(status, target, source, effect) {
			return this.dex.abilities.get('thermalexchange').onSetStatus?.call(this, status, target, source, effect);
		},
		onWeather(target, source, effect) {
			return this.dex.abilities.get('icebody').onWeather?.call(this, target, source, effect);
		},
		onResidual(pokemon) {
			this.dex.abilities.get('thermalexchange').onResidual?.call(this, pokemon);
			return this.dex.abilities.get('icebody').onResidual?.call(this, pokemon);
		},
		onImmunity(type, pokemon) {
			return this.dex.abilities.get('icebody').onImmunity?.call(this, type, pokemon);
	...
	moonveil: {
		onStart(pokemon) {
			this.dex.abilities.get('pastelveil').onStart?.call(this, pokemon);
			this.dex.abilities.get('mistysurge').onStart?.call(this, pokemon);
		},
		onSwitchIn(pokemon) {
			this.dex.abilities.get('pastelveil').onStart?.call(this, pokemon);
			this.dex.abilities.get('mistysurge').onStart?.call(this, pokemon);
		...
		onAnySwitchIn(pokemon) {
			...
			// Cure a newly arrived ally without resetting Misty Terrain.
			this.dex.abilities.get('pastelveil').onStart?.call(this, this.effectState.target);
		},
		onModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('pastelveil').onModifyDamage?.call(this, damage, source, target, move);
		},
		...
		onUpdate(pokemon) {
			return this.dex.abilities.get('pastelveil').onUpdate?.call(this, pokemon);
		},
		onSetStatus(status, target, source, effect) {
			return this.dex.abilities.get('pastelveil').onSetStatus?.call(this, status, target, source, effect);
		},
		onAllySetStatus(status, target, source, effect) {
			return this.dex.abilities.get('pastelveil').onAllySetStatus?.call(this, status, target, source, effect);
		},
		flags: {breakable: 1},
		name: "Moon Veil",
		rating: 4,
	...
	livewire: {
		...
		onModifySpe(spe, pokemon) {
			return this.dex.abilities.get('quickfeet').onModifySpe?.call(this, spe, pokemon);
		},
		onModifyAtkPriority: 5,
		onModifyAtk(atk, attacker, defender, move) {
			return this.dex.abilities.get('transistor').onModifyAtk?.call(this, atk, attacker, defender, move);
		},
		onModifySpAPriority: 5,
		onModifySpA(spa, attacker, defender, move) {
			return this.dex.abilities.get('transistor').onModifySpA?.call(this, spa, attacker, defender, move);
		},
		onSourceModifyAtkPriority: 5,
		onSourceModifyAtk(atk, attacker, defender, move) {
			return this.dex.abilities.get('transistor').onSourceModifyAtk?.call(this, atk, attacker, defender, move);
		},
		onSourceModifySpAPriority: 5,
		onSourceModifySpA(spa, attacker, defender, move) {
			return this.dex.abilities.get('transistor').onSourceModifySpA?.call(this, spa, attacker, defender, move);
		},
		onDamagingHitOrder: 1,
		onDamagingHit(damage, target, source, move) {
			return this.dex.abilities.get('ironbarbs').onDamagingHit?.call(this, damage, target, source, move);
		},
		flags: { breakable: 1 },
		name: "Livewire",
		rating: 5,
	...
	unovawing: {
		...
		onAfterEachBoost(boost, target, source, effect) {
			if (target.isAlly(source) || this.field.isTerrain('chessboardterrain')) return;
	...
	aevianwing: {
		onStart(pokemon) {
			if (pokemon.species.id !== 'unfezant' || pokemon.abilityState.aevianWingTransformed) return;
			pokemon.abilityState.aevianWingTransformed = true;
	...
	aeviantoxin: {
		onStart(pokemon) {
			if (pokemon.species.id === 'drapion') {
				pokemon.formeChange('Drapion-Rejuv', this.effect, true, '0');
			}
		},
		onBasePowerPriority: 19,
		onBasePower(basePower, attacker, defender, move) {
	...
	aevianspark: {
		onStart(pokemon) {
			if (pokemon.species.id === 'breloom') {
				pokemon.formeChange('Breloom-Rejuv', this.effect, true, '0');
			}
		},
		onBasePowerPriority: 30,
		onBasePower(basePower, attacker, defender, move) {
	...
	aevianrocket: {
		onStart(pokemon) {
			if (pokemon.species.id === 'veluza') {
				pokemon.formeChange('Veluza-Rejuv', this.effect, true, '0');
			}
	...
	adaptivecore: {
		...
		onCheckShow(pokemon) {
			return this.dex.abilities.get('selfrepair').onCheckShow?.call(this, pokemon);
		},
		onSwitchOut(pokemon) {
			return this.dex.abilities.get('selfrepair').onSwitchOut?.call(this, pokemon);
		},
		onResidual(pokemon) {
	...
	crueltag: {
		...
		onFoeTrapPokemon(pokemon) { return this.dex.abilities.get('shadowtag').onFoeTrapPokemon?.call(this, pokemon); },
		onFoeMaybeTrapPokemon(pokemon, source) { return this.dex.abilities.get('shadowtag').onFoeMaybeTrapPokemon?.call(this, pokemon, source); },
		onSourceModifyDamage(damage, source, target, move) {
	...
	cruelshell: {
		...
		onAfterEachBoost(boost, target, source, effect) {
			this.dex.abilities.get('shellarmor').onAfterEachBoost?.call(this, boost, target, source, effect);
		},
		onDamage(damage, target, source, effect) {
	...
	windchime: {
		...
		onSourceModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('punkrock').onSourceModifyDamage?.call(this, damage, source, target, move);
		},
	...
	auramaster: {
		onBasePowerPriority: 30,
		onBasePower(basePower, attacker, defender, move) {
			let componentBoost = 1;
			const minimum = this.field.isTerrain('factoryterrain') ? 80 : 60;
			if (this.modify(basePower, this.event.modifier) <= minimum) componentBoost *= 1.5;
	...
	apexvenom: {
		...
		onBasePowerPriority: 20,
		onBasePower(basePower, source, target, move) {
			if (move.id === 'poisonfang') return this.chainModify(1.5);
	...
	sirius: {
		...
		onBasePowerPriority: 21,
		onBasePower(basePower, source, target, move) {
			const apexPower = this.dex.abilities.get('apexvenom').onBasePower?.call(this, basePower, source, target, move);
			return this.dex.abilities.get('whiplash').onBasePower?.call(this, apexPower ?? basePower, source, target, move) ?? apexPower;
		},
		onResidual(pokemon) {
			this.dex.abilities.get('apexvenom').onResidual?.call(this, pokemon);
		},
		flags: { breakable: 1 },
		name: "Sirius",
		rating: 5,
	...
	aquashell: {
  onUpdate(pokemon) { this.dex.abilities.get('waterveil').onUpdate?.call(this, pokemon); },
  onSwitchIn(pokemon) { this.dex.abilities.get('waterveil').onSwitchIn?.call(this, pokemon); },
  onSetStatus(status, target, source, effect) { return this.dex.abilities.get('waterveil').onSetStatus?.call(this, status, target, source, effect); },
  ...
  flags: {breakable: 1}, name: 'Aqua Shell', rating: 5, num: 10180,
	...
	battlebond: {
		onSourceAfterFaint(length, target, source, effect) {
			...
			if (['arcaninebattlebond', 'garchompbattlebond', 'greninjaash'].includes(source.species.id)) {
		...
		onBasePower(basePower, source, target, move) {
			let modifier = 1;
			if (target?.hasAbility(['royaldecree', 'empress', 'neutralization'])) {
	...
	shadowbond: {
		...
		onImmunity(type, pokemon) { return this.dex.abilities.get('battlebond').onImmunity?.call(this, type, pokemon); },
		onResidual(pokemon) { return this.dex.abilities.get('battlebond').onResidual?.call(this, pokemon); },
		onDamage(damage, target, source, effect) {
			return this.dex.abilities.get('battlebond').onDamage?.call(this, damage, target, source, effect);
		},
		flags: { failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1 },
		name: "Shadow Bond",
		rating: 4,
	...
	apexbond: {
		...
		onDamagingHitOrder: 1,
		onDamagingHit(damage, target, source, move) {
			return this.dex.abilities.get('roughskin').onDamagingHit?.call(this, damage, target, source, move);
		},
		onSourceModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('battlebond').onSourceModifyDamage?.call(this, damage, source, target, move);
		},
		...
		onResidual(pokemon) { return this.dex.abilities.get('battlebond').onResidual?.call(this, pokemon); },
		onDamage(damage, target, source, effect) {
			const result = this.dex.abilities.get('battlebond').onDamage?.call(this, damage, target, source, effect);
			if (result !== undefined) return result;
	...
	sacredbond: {
		onStart(pokemon) {
			...
			if (!hasAdjacentFoe) {
				...
				pokemon.abilityState.sacredBondPendingIntimidate = true;
			}
		},
		onAnySwitchIn() {
		...
		onModifyMove(move) {
			if (move.id === 'extremespeed') move.willCrit = true;
		},
		onSourceModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('battlebond').onSourceModifyDamage?.call(this, damage, source, target, move);
		},
		onImmunity(type, pokemon) {
			const result = this.dex.abilities.get('magmaarmor').onImmunity?.call(this, type, pokemon);
			if (result === false) return false;
			return this.dex.abilities.get('battlebond').onImmunity?.call(this, type, pokemon);
		},
		onResidual(pokemon) {
			this.dex.abilities.get('battlebond').onResidual?.call(this, pokemon);
			this.dex.abilities.get('flashfire').onResidual?.call(this, pokemon);
		},
		onDamage(damage, target, source, effect) {
			return this.dex.abilities.get('battlebond').onDamage?.call(this, damage, target, source, effect);
		},
	...
	berserk: {
		onStart(pokemon) {
			if (this.field.isTerrain('dragonsdenterrain')) {
				this.boost({ atk: 2, spa: 2 }, pokemon, pokemon);
			}
		},
		onDamage(damage, target, source, effect) {
  ...
  onImmunity(type, pokemon) { return this.dex.abilities.get('selfsufficient').onImmunity?.call(this, type, pokemon); },
	...
	chlorophyll: {
		onModifySpe(spe, pokemon) {
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather()) || this.field.isTerrain('flowergarden4')) {
  ...
  flags: {}, name: 'Corrosive Touch', rating: 4.5, num: 11103,
	...
	defeatist: {
		...
		onImmunity(type, pokemon) {
			return this.dex.abilities.get('relicarmor').onImmunity?.call(this, type, pokemon);
		},
		onType(types, pokemon) {
			return this.dex.abilities.get('relicarmor').onType?.call(this, types, pokemon);
		},
		onResidual(pokemon) {
			return this.dex.abilities.get('relicarmor').onResidual?.call(this, pokemon);
		},
		onAfterBoost(boost, target, source, effect) {
			return this.dex.abilities.get('relicarmor').onAfterBoost?.call(this, boost, target, source, effect);
		},
	...
	eclipsevision: {
		...
		onStart(pokemon) {
			const firstMove = pokemon.moveSlots[0] && this.dex.moves.get(pokemon.moveSlots[0].id);
			if (!firstMove || !['Psychic', 'Dark'].includes(firstMove.type)) return;
			if (!pokemon.hasType(firstMove.type) && pokemon.setType(firstMove.type)) {
				this.add('-start', pokemon, 'typechange', firstMove.type, '[from] ability: Eclipse Vision');
			}
		},
		...
		onResidual(pokemon) {
			if (pokemon.hasType('Psychic')) this.heal(pokemon.baseMaxhp / 8, pokemon, pokemon);
		},
		flags: { breakable: 1 },
		name: "Eclipse Vision",
		rating: 4,
	...
	venomarmor: {
		...
		onBasePowerPriority: 20,
		onBasePower(basePower, source, target, move) {
			if (move.id === 'metalclaw') return this.chainModify(1.5);
			if (move.multihitType === 'dualwield') return this.chainModify(getDualWieldModifier(move));
		},
		onModifyDamage(damage, source, target, move) {
			if (source.status && move.category === 'Physical') return this.chainModify(1.3);
		},
		onDamage(damage, target, source, effect) {
	...
	toxicarmor: {
		...
		onBasePowerPriority: 20,
		onBasePower(basePower, source, target, move) {
			return this.dex.abilities.get('venomarmor').onBasePower?.call(this, basePower, source, target, move);
		},
		onModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('venomarmor').onModifyDamage?.call(this, damage, source, target, move);
		},
		onDamage(damage, target, source, effect) {
			return this.dex.abilities.get('venomarmor').onDamage?.call(this, damage, target, source, effect);
		},
		onResidual(pokemon) {
			return this.dex.abilities.get('venomarmor').onResidual?.call(this, pokemon);
		},
		onModifySpe(spe, pokemon) {
			return this.dex.abilities.get('violentrush').onModifySpe?.call(this, spe, pokemon);
		},
	...
	noseformation: {
		...
		onSourceDamagingHit(damage, target, source, move) {
			...
			for (let i = 0; i < 3; i++) {
				...
				const bestMove = (this.effect as any).getBestNoseMove.call(this, source, foe) as ActiveMove | null;
				if (!bestMove) break;
				...
				if (typeof noseDamage === 'number' && noseDamage > 0) {
					...
					if (!foe.hp) {
						const bestStat = source.getBestStat(true, true);
						this.boost({ [bestStat]: 1 }, source, source, this.dex.abilities.get('elevate'));
					}
				}
			}
		},
	...
	fallenstar: {
		...
		onSourceAfterFaint(length, target, source, effect) {
			...
			newMove.ignoreAbility = true;
			const followTarget = source.foes().find(foe => this.actions.canChainHitTarget(foe, source, newMove));
	...
	eclipse: {
		onStart(pokemon) {
			if (['espeon', 'umbreon', 'eeveestarter', 'eeveestarteralt'].includes(pokemon.species.id)) {
				pokemon.formeChange('Soluneon', this.effect, false, '[msg]');
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, source, target, move) {
	...
	ragingstorm: {
		...
		onSourceModifyDamage(damage, source, target, move) {
			this.dex.abilities.get('battlearmor').onSourceModifyDamage?.call(this, damage, source, target, move);
	...
	ragingoverlord: {
		onCriticalHit: false,
		onAfterEachBoost(boost, target, source, effect) {
			return this.dex.abilities.get('ragingstorm').onAfterEachBoost?.call(this, boost, target, source, effect);
		},
		fallen(pokemon) {
			return this.dex.abilities.get('supremeoverlord').fallen?.call(this, pokemon) || 0;
		},
		onStart(pokemon) {
			this.dex.abilities.get('ragingstorm').onStart?.call(this, pokemon);
	...
	soulstrike: {
		...
		onTryHit(target, source, move) {
			if (target !== source && this.movehasType(move, 'Ghost')) {
				if (!this.heal(target.baseMaxhp / 4)) this.add('-immune', target, '[from] ability: Soul Strike');
				return null;
			}
		},
		onFaint(pokemon) {
			if (this.field.terrain === 'hauntedterrain') {
				this.field.setTerrainDuration(Math.max(this.field.terrainState.duration || 0, 5));
			} else if (this.field.setTerrain('hauntedterrain', pokemon, this.dex.abilities.get('soulstrike'), true)) {
				this.field.setTerrainDuration(5);
			}
		},
		flags: { cantsuppress: 1 },
	...
	hellfireeclipse: {
		onStart(pokemon) {
			this.dex.abilities.get('darkaura').onStart?.call(this, pokemon);
		},
		onAnyBasePowerPriority: 20,
		onAnyBasePower(basePower, source, target, move) {
			return this.dex.abilities.get('darkaura').onAnyBasePower?.call(this, basePower, source, target, move);
		},
		onImmunity(type, pokemon) {
	...
	curseddoll: {
		...
		onSourceDamagingHit(damage, target, source, move) {
			if (!damage || !target || target.fainted || !source || target === source || target.isAlly(source) || move.category === 'Status') return;
			target.addVolatile('curse', source, this.dex.abilities.get('curseddoll'));
		},
		onFaint(pokemon) {
			if (this.field.terrain === 'hauntedterrain') {
				this.field.setTerrainDuration(Math.max(this.field.terrainState.duration || 0, 5));
			} else if (this.field.setTerrain('hauntedterrain', pokemon, this.dex.abilities.get('curseddoll'), true)) {
				this.field.setTerrainDuration(5);
			}
		},
		flags: {},
	...
	cursedmarionette: {
		...
		onDamagingHit(damage, target, source, move) {
			...
			if (this.field.terrain === 'hauntedterrain') {
				...
				this.field.setTerrainDuration(3);
			}
		},
		onFaint(pokemon) {
			for (const foe of pokemon.foes()) {
			...
			if (this.field.terrain === 'hauntedterrain') {
				this.field.setTerrainDuration((this.field.terrainState.duration || 0) + 5);
	...
	sandsovereign: {
		...
		onResidual(pokemon) {
			for (const target of pokemon.foes()) {
				...
				this.damage(abilityChipDamage(this, target, 'Rock'), target, pokemon);
			}
		},
		flags: { breakable: 1 },
		name: "Sand Sovereign",
	...
	frostsovereign: {
		...
		onResidual(pokemon) {
			...
			for (const target of pokemon.foes()) {
				...
				this.damage(abilityChipDamage(this, target, 'Ice'), target, pokemon);
			}
		},
		flags: { breakable: 1 },
	...
	mirrorgreed: {
		onStart(pokemon) { return this.dex.abilities.get('magicbounce').onStart?.call(this, pokemon); },
		onBasePower(basePower, attacker, defender, move) {
	...
	royalvoice: {
		...
		onAnyBoost(boost, target, source, effect) {
			return this.dex.abilities.get('dreamsickness').onAnyBoost?.call(this, boost, target, source, effect);
		},
  ...
  onAnyDamage(damage, target, source, effect) { return this.dex.abilities.get('dreamsickness').onAnyDamage?.call(this, damage, target, source, effect); },
  onResidual(pokemon) { this.dex.abilities.get('dreamsickness').onResidual?.call(this, pokemon); },
		onModifyTypePriority: -1,
		onModifyType(move, pokemon) {
			const noModifyType = [
				'judgment', 'multiattack', 'naturalgift', 'revelationdance', 'technoblast', 'terrainpulse', 'weatherball',
	...
	adaptivecell: {
		onStart(pokemon) {
			const firstMove = pokemon.moveSlots[0] && this.dex.moves.get(pokemon.moveSlots[0].id);
			if (!firstMove || !['Physical', 'Special'].includes(firstMove.category)) return;
			const newType = firstMove.category === 'Physical' ? 'Fighting' : 'Psychic';
	...
	perfectforesight: {
		...
		onStart(pokemon) {
			...
			const speedConditions: {[id: string]: boolean} = {
				...
				swiftswim: ['raindance', 'primordialsea'].includes(weather) || this.field.isTerrain(['watersurfaceterrain', 'underwaterterrain', 'murkwatersurfaceterrain', 'midnightzoneterrain']),
				...
				speedboost: true,
				telepathy: this.field.isTerrainOrAura('psychicterrain'),
			};
			for (const target of pokemon.foes()) {
			...
			if (best) {
				...
				this.add('-ability', pokemon, ability.name, `[from] ability: ${pokemon.getAbility().name}`);
  ...
  onImmunity(type, pokemon) { return this.dex.abilities.get('icebody').onImmunity?.call(this, type, pokemon); },
	...
	treasuretitan: {
		...
		onBasePower(basePower, source, target, move) {
			const baseMove = move.baseMove || move.id;
			if (source.species.id === 'copperajahgmax' && WEIGHT_BASE_POWER_MOVES.has(baseMove)) {
				return Math.max(basePower, 120);
			}
		},
		onDamage(damage, target, source, effect) {
	...
	heatcoil: {
		onStart(pokemon) { this.dex.abilities.get('flamebody').onStart?.call(this, pokemon); },
		onDamagingHit(damage, target, source, move) {
			return this.dex.abilities.get('flamebody').onDamagingHit?.call(this, damage, target, source, move);
		},
		onResidual(pokemon) {
	...
	sweetsanctuary: {
		...
		onUpdate(pokemon) { this.dex.abilities.get('pastelveil').onUpdate?.call(this, pokemon); },
		onSetStatus(status, target, source, effect) {
			return this.dex.abilities.get('pastelveil').onSetStatus?.call(this, status, target, source, effect);
		},
		onAnyModifyDamage(damage, source, target, move) {
		...
		onAllySetStatus(status, target, source, effect) {
			const pastelVeilResult = this.dex.abilities.get('pastelveil').onAllySetStatus?.call(this, status, target, source, effect);
			if (pastelVeilResult !== undefined) return pastelVeilResult;
  ...
  flags: {}, name: 'Sacred Power', rating: 5, num: 11099,
	...
	electricsurge: {
		onStart(source) {
			this.field.setFieldOrAura('electricterrain', source.hasItem('amplifieldrock') ? 8 : 5, source, this.effect);
		},
		flags: {},
		name: "Electric Surge",
		rating: 4,
	...
	fluffyevo: {
		...
		onTryHit(target, source, move) {
			if (move.flags['powder'] && target !== source && this.dex.getImmunity('powder', target)) {
				this.add('-immune', target, '[from] ability: Fluffy Evo');
				return null;
			}
		},
		onModifyMove(move, pokemon) {
	...
	mindfreeze: {
		onStart(pokemon) {
			if (['espeon', 'glaceon', 'eeveestarter', 'eeveestarteralt'].includes(pokemon.species.id)) {
				pokemon.formeChange('Auroreon', this.effect, false, '[msg]');
			}
		},
		onTryHit(target, source, move) {
	...
	tyrantdomain: {
		...
		onStart(pokemon) {
			...
			(this.dex.abilities.get('supremeoverlord') as any).onStart?.call(this, pokemon);
		},
		onFaint(pokemon) {
		...
		onImmunity(type, pokemon) { return (this.dex.abilities.get('relicarmor') as any).onImmunity?.call(this, type, pokemon); },
		onAfterBoost(boost, target, source, effect) {
		...
		onSourceModifyDamage(damage, source, target, move) {
			...
			return (this.dex.abilities.get('supremeoverlord') as any).onSourceModifyDamage?.call(this, damage, source, target, move);
		},
		onResidual(pokemon) { return (this.dex.abilities.get('selfsufficient') as any).onResidual?.call(this, pokemon); },
		onDamage(damage, target, source, effect) {
			return (this.dex.abilities.get('supremeoverlord') as any).onDamage?.call(this, damage, target, source, effect);
		},
		flags: {},
		name: "Tyrant Domain",
		rating: 5,
	...
	auroradomain: {
		...
		onSourceModifyDamage(damage, source, target, move) {
			return (this.dex.abilities.get('relicarmor') as any).onSourceModifyDamage?.call(this, damage, source, target, move);
		},
		onResidual(pokemon) { return (this.dex.abilities.get('selfsufficient') as any).onResidual?.call(this, pokemon); },
		flags: {},
		name: "Aurora Domain",
	...
	royalscales: {
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			return (this.dex.abilities.get('dragonize') as any).onBasePower?.call(this, basePower, pokemon, target, move);
		},
		onModifySTAB(stab, source, target, move) {
	...
	truedevotion: {
		...
		onCheckShow(pokemon) {
			return this.dex.abilities.get('falsedevotion').onCheckShow?.call(this, pokemon);
		},
		onSwitchOut(pokemon) {
	...
	pollenbloom: {
		...
		onResidual(pokemon) {
			for (const target of this.getAllActive()) {
				...
				const damage = abilityChipDamage(this, target, 'Grass');
	...
	plasmaeruption: {
		onModifyMove(move, source, target) {
			if (move.id === 'burnup' || move.id === 'doubleshock' || (move as any).plasmaEruptionChecked) return;
			(move as any).plasmaEruptionChecked = true;
			const fire = source.hasType('Fire');
			const electric = source.hasType('Electric');
			let newType: 'Fire' | 'Electric' | '' = '';
			if (!fire && electric && move.type === 'Fire') {
				newType = 'Electric';
			} else if (fire && !electric && move.type === 'Electric') {
				newType = 'Fire';
			} else if (fire && electric && this.randomChance(1, 2)) {
				if (move.type === 'Fire') newType = 'Electric';
				if (move.type === 'Electric') newType = 'Fire';
			}
			if (!newType) return;
			const possibleTargets = target ? [target] : source.foes();
			if (possibleTargets.some(foe => foe.hp && (
				!foe.runImmunity(newType, false) ||
				(newType === 'Electric' && foe.hasAbility(['voltabsorb', 'lightningrod', 'motordrive'])) ||
				(newType === 'Fire' && foe.hasAbility(['flashfire', 'wellbakedbody']))
			))) return;
			move.type = newType;
			if (!fire && newType === 'Electric') {
				this.add('-message', 'Plasma Eruption turned Fire into Electricity!');
			} else if (!electric && newType === 'Fire') {
	...
	waterbarrage: {
		...
		onResidual(pokemon) {
			...
			for (const target of this.getAllActive()) {
				...
				const damage = abilityChipDamage(this, target, 'Water', stage);
	...
	flashfire: {
		...
		condition: {
			...
			onModifyAtkPriority: 5,
			onModifyAtk(atk, attacker, defender, move) {
				if (move && this.movehasType(move, 'Fire') && attacker.hasAbility(['flashfire', 'livinglegend', 'kindledfury'])) {
	...
	flowergift: {
		...
		onWeatherChange(pokemon) {
			...
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather()) || this.field.isTerrain('flowergarden1')) {
		...
		onTerrainChange(pokemon) {
			if (!pokemon.isActive || pokemon.baseSpecies.baseSpecies !== 'Cherrim' || pokemon.transformed) return;
			if (!pokemon.hp) return;
			if (this.field.isTerrain(['bewitchedwoodsterrain', 'flowergarden1']) || ['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather())) {
				if (pokemon.species.id !== 'cherrimsunshine') {
					pokemon.formeChange('Cherrim-Sunshine', this.effect, false, '[msg]');
				}
			} else {
	...
	phantombarrage: {
		...
		onBasePower(basePower, source, target, move) {
			const hydraBondPower = this.dex.abilities.get('hydrabond').onBasePower?.call(this, basePower, source, target, move);
			if (move.id === 'dragondarts' || move.id === 'gmaxspiritvolley') {
				return this.chainModify(1.2);
			}
			return hydraBondPower;
		},
		onImmunity(type, pokemon) {
			if (type === 'Ground') return false;
		},
	...
	astralcore: {
		...
		onResidual(pokemon) {
			return this.dex.abilities.get('naturalcure').onResidual?.call(this, pokemon);
		},
		flags: {},
		name: "Astral Core",
		rating: 5,
	...
	desertspirit: {
		...
		onModifyMove(move) {
			if (move.category !== 'Status' && this.movehasType(move, 'Ground')) move.forceSTAB = true;
		},
		onModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('tintedlens').onModifyDamage?.call(this, damage, source, target, move);
		},
	...
	desertshell: {
		...
		onDamage(damage, target, source, effect) {
			return this.dex.abilities.get('heatproof').onDamage?.call(this, damage, target, source, effect);
		},
		flags: { breakable: 1 },
		name: "Desert Shell",
		rating: 5,
	...
	wildfirecore: {
		...
		onResidual(pokemon) {
			for (const target of this.getAllActive()) {
				...
				const damage = abilityChipDamage(this, target, 'Fire', multiplier);
				this.damage(damage, target, pokemon);
			}
		},
		flags: { breakable: 1 },
	...
	lingeringaroma: {
		onResidualOrder: 28,
		onResidual(pokemon) {
			if (this.field.flowerGardenStage() < 3) return;
			for (const foe of pokemon.foes()) {
				const stat = this.sample(['def', 'spd', 'spe'] as const);
				this.boost({ [stat]: -1 }, foe, pokemon, this.effect);
			}
		},
		onDamagingHit(damage, target, source, move) {
	...
	lunarorbit: {
		onStart(pokemon) {
			...
			if (gravityWasActive) this.field.applyGravityTerrainChange(pokemon, this.effect);
		},
		onDamage(damage, target, source, effect) {
			return this.dex.abilities.get('magicguard').onDamage?.call(this, damage, target, source, effect);
		},
	...
	prismscale: {
		...
		onUpdate(pokemon) {
			if (pokemon.volatiles['attract']) {
				this.add('-activate', pokemon, 'ability: Prism Scale');
				pokemon.removeVolatile('attract');
				this.add('-end', pokemon, 'move: Attract', '[from] ability: Prism Scale');
			}
		...
		onTryHit(pokemon, target, move) {
			if (['attract', 'captivate', 'taunt'].includes(move.id)) {
				this.add('-immune', pokemon, '[from] ability: Prism Scale');
				return null;
			}
		},
		onTryBoost(boost, target, source, effect) {
			if (effect.name === 'Intimidate' && boost.atk) {
				delete boost.atk;
				this.add('-fail', target, 'unboost', 'Attack', '[from] ability: Prism Scale', `[of] ${target}`);
			}
		},
		onModifySpe(spe, pokemon) {
			return this.dex.abilities.get('swiftswim').onModifySpe?.call(this, spe, pokemon);
	...
	heavyartillery: {
		...
		onBasePowerPriority: 19,
		onBasePower(basePower, attacker, defender, move) {
			if (move.category !== 'Status' && (move.flags['pulse'] || move.flags['bullet'])) {
				return this.chainModify(2);
			}
		},
		onModifyMove(move) {
	...
	bloomingsun: {
		onWeatherModifyDamage(damage, attacker, defender, move) {
			return this.dex.abilities.get('megasol').onWeatherModifyDamage?.call(this, damage, attacker, defender, move);
		},
		onBasePowerPriority: 22,
		onBasePower(basePower, attacker, defender, move) {
	...
	*/
	...
	mimicry: {
		...
		onTerrainChange(pokemon) {
			...
			if (this.field.flowerGardenStage()) {
				newType = 'Grass';
	...
	stormbell: {
		onStart(pokemon) {
			this.dex.abilities.get('drizzle').onStart?.call(this, pokemon);
			this.dex.abilities.get('mirrorarmor').onStart?.call(this, pokemon);
		},
		onTryBoost(boost, target, source, effect) {
		...
		onSourceModifyDamage(damage, source, target, move) {
			return this.dex.abilities.get('mirrorarmor').onSourceModifyDamage?.call(this, damage, source, target, move);
		},
		onSourceAfterFaint(length, target, source, effect) {
			return this.dex.abilities.get('elevate').onSourceAfterFaint?.call(this, length, target, source, effect);
		},
		flags: {breakable: 1}, name: 'Storm Bell', rating: 5, num: 11092,
	},
	mirrorarmor: {
		onStart(pokemon) {
			if (this.field.isTerrain('fairytaleterrain')) {
	...
	requiem: {
		markRequiem(target, source) {
			if (!target || target.fainted || !source || target.isAlly(source) || target.volatiles['requiem']) return;
			if (!target.addVolatile('requiem', source, this.dex.abilities.get('requiem'))) return;
			if (!target.volatiles['perishsong'] &&
				target.addVolatile('perishsong', source, this.dex.abilities.get('requiem'))) {
				this.add('-start', target, 'perish3', '[from] ability: Requiem', `[of] ${source}`);
			}
		},
		onSourceDamagingHit(damage, target, source, move) {
			(this.effect as any).markRequiem.call(this, target, source);
		},
		onAnyFaint(fainted) {
			const pokemon = this.effectState.target;
			if (!pokemon || pokemon.fainted || fainted === pokemon || fainted.isAlly(pokemon)) return;
			this.heal(pokemon.baseMaxhp / 4, pokemon, pokemon);
		},
		onDamagingHit(damage, target, source, move) {
			if (!source || source.fainted) return;
			(this.effect as any).markRequiem.call(this, source, target);
  ...
  onDamagingHit(damage, target) {
   ...
   if (this.field.terrain === 'hauntedterrain') {
	this.field.setTerrainDuration(Math.max(this.field.terrainState.duration || 0, 3));
   } else if (this.field.setTerrain('hauntedterrain', target, this.dex.abilities.get('reapersgrip'), true)) {
	this.field.setTerrainDuration(3);
   }
  },
  onFaint(pokemon) {
   if (this.field.terrain === 'hauntedterrain') {
	this.field.setTerrainDuration((this.field.terrainState.duration || 0) + 5);
   } else if (this.field.setTerrain('hauntedterrain', pokemon, this.dex.abilities.get('reapersgrip'), true)) {
	this.field.setTerrainDuration(5);
   }
  },
  flags: {cantsuppress: 1}, name: "Reaper's Grip", rating: 5, num: 10357,
	...
	neutralizinggas: {
		...
		onSwitchIn(pokemon) {
			...
			for (const target of this.getAllActive()) {
				...
				target.revertSuppressedRegionalForm();
				if (target.illusion) {
	...
	predator: {
		...
		onBasePower(basePower, pokemon, target, move) {
			if (target.hasAbility('neutralization') || (target.hasAbility(['royaldecree', 'empress']) && !this.getAllActive().some(pokemon => pokemon.hasAbility('neutralization')))) {
	...
	pressure: {
		onStart(pokemon) {
			if (this.field.isTerrain('underwaterterrain')) this.field.changeTerrain('midnightzoneterrain', pokemon, this.effect);
			if (this.field.isTerrain('coldeclipseterrain')) {
				let activated = false;
				for (const target of pokemon.foes()) {
					if (!activated) {
		...
		onDeductPP(target, source) {
			if (target.isAlly(source)) return;
			return this.field.isTerrain('midnightzoneterrain') ? 2 : 1;
		},
		flags: {},
		name: "Pressure",
		rating: 2.5,
	...
	propellertail: {
		...
		onModifySpe(spe, pokemon) {
			if (this.field.isTerrain('watersurfaceterrain') || this.field.isTerrain(['underwaterterrain', 'midnightzoneterrain'])) {
				return this.chainModify(2);
			}
		},
		flags: {},
	...
	zprotean: {
		onModifyPriority(priority, pokemon, target, move) {
			...
			applyZProteanForm(this, pokemon, move, this.effect);
		},
		onPrepareHit(source, target, move) {
			applyZProteanForm(this, source, move, this.effect);
		},
		onSwitchOut(pokemon) {
			pokemon.zProteanForme = undefined;
			pokemon.zProteanVisualSpecies = undefined;
		},
		flags: {},
		name: "Z Protean",
		rating: 4,
	...
	solarrush: {
		onModifySpe(spe, pokemon) {
			const sandSpeed = this.dex.abilities.get('sandrush').onModifySpe?.call(this, spe, pokemon);
			return this.dex.abilities.get('chlorophyll').onModifySpe?.call(this, sandSpeed ?? spe, pokemon) ?? sandSpeed;
		},
		onImmunity(type, pokemon) {
			return this.dex.abilities.get('sandrush').onImmunity?.call(this, type, pokemon);
		},
		flags: {},
		name: "Solar Rush",
		rating: 4,
	...
	schooling: {
		...
		onStart(pokemon) {
			...
			const forcedSchool = this.field.isTerrain(['underwaterterrain', 'midnightzoneterrain']) ||
	...
	seviischooling: {
		...
		onStart(pokemon) {
			...
			const forcedSchool = this.field.isTerrain(['underwaterterrain', 'midnightzoneterrain']) ||
				(this.field.isTerrain(['watersurfaceterrain', 'murkwatersurfaceterrain']) && pokemon.isGrounded());
			const school = pokemon.level >= 20 && (forcedSchool || pokemon.hp > pokemon.maxhp / 4);
			const forme = school ? 'Wishiwashi-Sevii-Schooling' : 'Wishiwashi-Sevii';
		...
		onResidual(pokemon) {
			...
			const forme = school ? 'Wishiwashi-Sevii-Schooling' : 'Wishiwashi-Sevii';
			if (pokemon.species.name !== forme) pokemon.formeChange(forme);
			if (school) this.dex.abilities.get('selfrepair').onResidual?.call(this, pokemon);
		},
		flags: { failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1 },
	...
	goldentalons: {
		...
		onBasePowerPriority: 19,
		onBasePower(basePower, source, target, move) {
			const goodAsGold = this.dex.abilities.get('goodasgold').onBasePower?.call(this, basePower, source, target, move);
			return this.dex.abilities.get('sharpness').onBasePower?.call(this, goodAsGold ?? basePower, source, target, move);
		},
		flags: { breakable: 1 },
		name: "Golden Talons",
		rating: 5,
	...
	silkendecoy: {
		...
		onTryHit(target, source, move) {
			if (target.m.silkenDecoyCocoon && source !== target && move.category === 'Status') {
				this.add('-immune', target, '[from] ability: Silken Decoy');
				return null;
			}
		},
		onModifySecondaries(secondaries, target) {
			if (target.m.silkenDecoyCocoon || target.abilityState.silkenDecoyBlockedMove) return [];
		},
		onStart(pokemon) {
		...
		onDamage(damage, target, source, effect) {
			...
			target.abilityState.silkenDecoyBlockedMove = move.id;
	...
	solarrecharge: {
		onTryHit(target, source, move) {
			if (target !== source && this.movehasType(move, 'Fire')) {
				if (!this.heal(target.baseMaxhp / 4, target, source)) {
					this.add('-immune', target, '[from] ability: Solar Recharge');
				}
				return null;
			}
		},
		onModifySTAB(stab, source, target, move) {
			if (move.type === 'Fire' && !source.hasType('Fire')) return 1.5;
		},
		onResidual(pokemon) {
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather())) {
				this.heal(pokemon.baseMaxhp / 8, pokemon, pokemon);
	...
	sinisterblaze: {
		onStart(pokemon) {
			if (['eeveestarter', 'eeveestarteralt'].includes(pokemon.species.id)) {
				pokemon.formeChange('Abysseon', this.effect, false, '[msg]');
			}
			if (pokemon.status !== 'brn') {
			...
			if (this.field.isTerrain([
				'fairytaleterrain', 'starlightarenaterrain', 'newworldterrain',
				'burningterrain', 'volcanicterrain', 'superheatedterrain', 'coldeclipseterrain',
			])) {
	...
	soulfire: {
		...
		onSourceModifyDamage(damage, source, target, move) {
			if (source?.hasAbility('soulstrike') && target.hasAbility('soulfire') && this.movehasType(move, 'Ghost')) {
				return this.chainModify(2);
			}
		...
		onTryHit(target, source, move) {
			if (source?.hasAbility('soulstrike') && this.movehasType(move, 'Ghost')) return;
		...
		onAnyRedirectTarget(target, source, source2, move) {
			...
			if (target?.hasAbility('soulstrike')) return;
			const redirectTarget = ['randomNormal', 'adjacentFoe'].includes(move.target) ? 'normal' : move.target;
			if (this.validTarget(this.effectState.target, source, redirectTarget)) {
				if (move.smartTarget) move.smartTarget = false;
				if (this.effectState.target !== target) {
	...
	forestsurge: {
		...
		onStart(source) {
			...
			if (this.field.setTerrain('forestterrain')) {
				this.field.setTerrainDuration(duration);
			}
			if (this.field.isTerrain('forestterrain')) {
				this.field.setAura('grassyterrain', duration, source, this.effect);
			}
		},
		flags: {},
		name: "Forest Surge",
	...
	parasitism: {
		...
		onDamage(damage, target, source, effect) {
			...
			if (damage >= target.hp && target.baseSpecies.baseSpecies === 'Parasect') {
	...
	completeparasitism: {
		onStart(pokemon) {
			if (pokemon.species.id === 'parasectmega') pokemon.m.parasectMegaUsed = true;
			return this.dex.abilities.get('parasitism').onStart?.call(this, pokemon);
		},
		onEffectiveness(typeMod, target, type, move) {
			return this.dex.abilities.get('parasitism').onEffectiveness?.call(this, typeMod, target, type, move);
		},
		onSourceBasePowerPriority: 17,
		onSourceBasePower(basePower, attacker, defender, move) {
			return this.dex.abilities.get('parasitism').onSourceBasePower?.call(this, basePower, attacker, defender, move);
		},
		onModifySecondaries(secondaries, target, source, move) {
			return this.dex.abilities.get('parasitism').onModifySecondaries?.call(this, secondaries, target, source, move);
		},
		onDamage(damage, target, source, effect) {
			return this.dex.abilities.get('parasitism').onDamage?.call(this, damage, target, source, effect);
		},
		onTryHitPriority: 1,
		onTryHit(target, source, move) {
		...
		onCheckShow(pokemon) { return this.dex.abilities.get('selfrepair').onCheckShow?.call(this, pokemon); },
		onSwitchOut(pokemon) { return this.dex.abilities.get('selfrepair').onSwitchOut?.call(this, pokemon); },
		onImmunity(type, pokemon) { return this.dex.abilities.get('selfrepair').onImmunity?.call(this, type, pokemon); },
		flags: { cantsuppress: 1 },
	...
	royaldecree: {
		...
		onAnyTryBoost(boost, target, source, effect) {
			...
			if (blocked) {
				const decree = this.getAllActive().find(pokemon => pokemon.hasAbility(['royaldecree', 'empress']));
				if (decree) {
		...
		onChargeMove(attacker) {
			if (this.getAllActive().some(pokemon => pokemon.hasAbility('neutralization'))) return;
			if (attacker.hasAbility(['royaldecree', 'empress'])) return false;
		},
		onModifyMove(move) {
			if (this.getAllActive().some(pokemon => pokemon.hasAbility('neutralization'))) return;
			if (move.flags['charge']) delete move.flags['charge'];
	...
	tremor: {
		onBasePowerPriority: 21,
		onBasePower(basePower, attacker, defender, move) {
			let modifier = move.flags['sound'] ? 1.5 : 1;
			if ((this.field.isWeather('sandstorm') || this.field.isTerrain('desertterrain') ||
				this.field.isTerrain('ashenbeachterrain')) &&
				(this.movehasType(move, 'Rock') || this.movehasType(move, 'Ground') || this.movehasType(move, 'Steel'))) {
				modifier *= 1.3;
			}
			if (modifier !== 1) return this.chainModify(modifier);
		},
		...
		onImmunity(type, pokemon) {
			if (type === 'Ground') return false;
			return this.dex.abilities.get('sandforce').onImmunity?.call(this, type, pokemon);
		},
		flags: {},
		name: "Tremor",
		rating: 4,
	...
	wrathshield: {
		...
		onStart(pokemon) {
			if (this.field.isTerrain(['coldeclipseterrain', 'newworldterrain', 'starlightarenaterrain', 'fairytaleterrain'])) {
				...
				this.boost({ def: 1 }, pokemon, pokemon, this.dex.abilities.get('wrathshield'));
			}
		},
		onCheckShow(pokemon) {
			this.dex.abilities.get('selfrepair').onCheckShow?.call(this, pokemon);
	...
	toxicbloom: {
		onStart(pokemon) { return this.dex.abilities.get('pollenbloom').onStart?.call(this, pokemon); },
		onAnyModifyBoost(boosts, pokemon) {
			return this.dex.abilities.get('pollenbloom').onAnyModifyBoost?.call(this, boosts, pokemon);
		},
		onImmunity(type, pokemon) {
	...
	froststalker: {
		...
		onBasePower(basePower, attacker, defender, move) {
			...
			this.dex.abilities.get('refrigerate').onBasePower?.call(this, basePower, attacker, defender, move);
		},
		flags: {}, name: 'Frost Stalker', rating: 5, num: 11096,
	},
	stakeout: {
		onModifyAtkPriority: 5,
	...
	steamengine: {
		...
		onStart(pokemon) {
			if (this.field.isTerrain('midnightzoneterrain') && pokemon.setType('Water')) {
				this.add('-start', pokemon, 'typechange', 'Water', '[from] ability: Steam Engine');
			}
  ...
  flags: {}, name: 'Night Hunt', rating: 4.5, num: 11101,
	...
	blackfang: {
		...
		onSourceAfterFaint(length, target, source, effect) {
			return this.dex.abilities.get('moxie').onSourceAfterFaint?.call(this, length, target, source, effect);
		},
		flags: {},
		name: "Black Fang",
		rating: 3.5,
		...
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				if (target.volatiles['substitute']) {
					...
					const stage = this.field.flowerGardenStage();
					const boosts: SparseBoostsTable = { evasion: -1 };
					if (stage >= 3) Object.assign(boosts, { def: -1, spd: -1 });
					if (stage >= 4) Object.assign(boosts, { atk: -1, spa: -1 });
					if (stage === 5) Object.assign(boosts, { spe: -1, accuracy: -1 });
					this.boost(boosts, target, pokemon, null, true);
				}
			}
	...
	swiftswim: {
		onModifySpe(spe, pokemon) {
			if (['raindance', 'primordialsea'].includes(pokemon.effectiveWeather()) || this.field.isTerrain('watersurfaceterrain') || this.field.isTerrain(['underwaterterrain', 'midnightzoneterrain']) || this.field.terrain === 'murkwatersurfaceterrain') {
				return this.chainModify(2);
			}
		},
		flags: {},
	...
	rainsovereign: {
		...
		onResidual(pokemon) {
			for (const target of pokemon.foes()) {
				...
				this.damage(abilityChipDamage(this, target, 'Water'), target, pokemon);
			}
		},
		flags: {},
		name: "Rain Sovereign",
	...
	phantomfist: {
		onBasePower(basePower, source, target, move) {
			return this.dex.abilities.get('unseenfist').onBasePower?.call(this, basePower, source, target, move);
		},
		onCheckShow(pokemon) {
			this.dex.abilities.get('selfrepair').onCheckShow?.call(this, pokemon);
		},
	...
	burningego: {
		onBasePowerPriority: 22,
		onBasePower(basePower, attacker, defender, move) {
			this.dex.abilities.get('ultraego').onBasePower?.call(this, basePower, attacker, defender, move);
	...
	watercompaction: {
		onStart(pokemon) {
			if (this.field.isTerrain('midnightzoneterrain') && pokemon.setType('Water')) {
				this.add('-start', pokemon, 'typechange', 'Water', '[from] ability: Water Compaction');
			}
	...
	whiplash: {
		...
		onBasePowerPriority: 21,
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags['tail'] || move.flags['tailmove']) return this.chainModify(1.5);
		},
	...
	zerotohero: {
		...
		onSwitchIn(pokemon) {
			if (pokemon.baseSpecies.baseSpecies !== 'Palafin') return;
			if (this.field.isTerrain(['murkwatersurfaceterrain', 'underwaterterrain', 'midnightzoneterrain', 'watersurfaceterrain'])) {
				if (pokemon.species.forme !== 'Hero') {
					pokemon.formeChange('Palafin-Hero', this.effect, true);
					pokemon.heroMessageDisplayed = false;
				}
	...
	empress: {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (type === 'Fairy' && move && ['Poison', 'Steel'].includes(move.type)) return typeMod - 1;
		},
		onModifySTAB(stab, source, target, move) {
	...
	loyalguard: {
		...
		onDragOutPriority: 1,
		onDragOut(pokemon) {
			return this.dex.abilities.get('guarddog').onDragOut?.call(this, pokemon);
		},
		onTryBoostPriority: 2,
		onTryBoost(boost, target, source, effect) {
			return this.dex.abilities.get('guarddog').onTryBoost?.call(this, boost, target, source, effect);
		},
		flags: {},
		name: "Loyal Guard",
		rating: 4,
	...
	islandcurrent: {
		...
		onResidual(pokemon) {
			this.dex.abilities.get('relicarmor').onResidual?.call(this, pokemon);
			return this.dex.abilities.get('windrider').onResidual?.call(this, pokemon);
		},
		flags: { breakable: 1 },
		name: "Island Current",
	...
	triplethreat: {
		...
		onModifyCritRatio(critRatio, source, target, move) {
			return this.dex.abilities.get('tangledfeet').onModifyCritRatio?.call(this, critRatio, source, target, move);
		},
		onModifyAccuracyPriority: -1,
		onModifyAccuracy(accuracy, target, source, move) {
