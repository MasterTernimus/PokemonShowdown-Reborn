...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	auramaster: {
		onBasePower(basePower, attacker, defender, move) {
			return this.dex.abilities.get('megalauncher').onBasePower?.call(this, basePower, attacker, defender, move);
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (move.category !== 'Status') return this.chainModify(0.65);
		},
	...
	bonewarrior: {
		...
		onBasePower(basePower, source, target, move) {
			const skillLinkMod = this.dex.abilities.get('skilllink').onBasePower?.call(this, basePower, source, target, move);
			if (skillLinkMod) return skillLinkMod;
			if (move.basePower <= 60) return this.chainModify(1.5);
		},
		onCriticalHit: false,
		flags: {},
		name: "Bone Warrior",
	...
	dualwield: {
		onModifyMove(move) {
			...
			if (!(move.flags['slicing'] || move.flags['pulse'] || move.flags['bullet'] || arrowMoves.includes(move.id))) return;
			move.multihit = 2;
	...
	apexpredator: {
		...
		onSourceAfterFaint(length, target, source, effect) {
			if (effect?.effectType === 'Move') this.heal(source.baseMaxhp / 4 * length, source, source);
		},
	...
	violentrush: {
		...
		onModifyAtk(atk, pokemon) {
			if (!pokemon.activeTurns) return this.chainModify(1.2);
		},
		flags: {},
		name: "Violent Rush",
		rating: 3.5,
	...
	solargrace: {
		...
		onTryBoost(boost, target, source, effect) {
			if (!source || target.isAlly(source)) return;
			let blocked = false;
			for (const stat of ['spe', 'spa'] as BoostID[]) {
				if (boost[stat] && boost[stat]! < 0) {
					delete boost[stat];
					blocked = true;
				}
			}
			if (blocked) this.add('-fail', target, 'unboost', '[from] ability: Solar Grace', `[of] ${target}`);
		},
		onBasePowerPriority: 21,
		onBasePower(basePower, source, target, move) {
			if (move.category === 'Status') return;
			if (!this.movehasType(move, ['Fire', 'Flying'])) return;
			if (target && (this.queue.willMove(target) || target.newlySwitched)) {
				this.debug('Solar Grace boost');
				return this.chainModify(1.2);
			}
		},
		onSourceModifyDamage(damage, source, target, move) {
	...
	burningspirit: {
		onUpdate(pokemon) {
			if (pokemon.status === 'frz') {
				this.add('-activate', pokemon, 'ability: Burning Spirit');
				pokemon.cureStatus();
			}
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (this.movehasType(move, 'Water')) return this.chainModify(0.5);
			if (move.flags['contact']) return this.chainModify(0.7);
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (pokemon.hp <= pokemon.maxhp / 2 && this.movehasType(move, 'Fire')) return this.chainModify(1.5);
		},
		onModifyMove(move) {
			if (move.category === 'Status') return;
		...
		onDamage(damage, target, source, effect) {
			if (target.abilityState.burningSpiritAwakened && effect?.effectType !== 'Move') return false;
			if (!target.abilityState.burningSpiritAwakened && effect?.effectType === 'Move' && damage >= target.hp) {
				target.abilityState.burningSpiritAwakened = true;
				this.add('-ability', target, 'Burning Spirit');
				this.add('-message', `${target.name}'s Burning Spirit awakened!`);
				return target.hp - 1;
			}
		},
		onResidual(pokemon) {
	...
	emperorsresolve: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, source, target, move) {
			if (this.movehasType(move, ['Water', 'Steel'])) return this.chainModify(1.2);
		},
		onModifyMove(move) {
	...
	terraresolve: {
		onAfterEachBoost(boost, target, source, effect) {
			...
			if (statsLowered) {
				...
				if (boost.def! < 0 || boost.spd! < 0) target.abilityState.terraResolveGuard = this.turn + 1;
			}
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (move.category === 'Status') return;
			if (target.abilityState.terraResolveGuard && target.abilityState.terraResolveGuard >= this.turn) return this.chainModify(0.75);
			if (target.getMoveHitData(move).typeMod > 0) return this.chainModify(0.75);
		},
		onSourceDamagingHit(damage, target, source, move) {
			if (move.category !== 'Status') this.heal(damage / 8, source, source);
		},
		onResidual(pokemon) {
			this.heal(pokemon.baseMaxhp / 16, pokemon, pokemon);
		},
	...
	firstvenom: {
		onModifyPriority(priority, pokemon, target, move) {
			if (pokemon.hp >= pokemon.maxhp * 3 / 4 && move.category !== 'Status' && this.movehasType(move, ['Poison', 'Dark'])) {
				return priority + 1;
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (pokemon.hp >= pokemon.maxhp * 3 / 4 && this.movehasType(move, 'Dark')) return this.chainModify(1.5);
		},
		onModifyMove(move, pokemon) {
	...
	noseformation: {
		onSourceDamagingHit(damage, target, source, move) {
			if (!target.hp || target.fainted) return;
			const targets = this.gameType === 'freeforall' ?
			...
			for (const foe of targets) {
				for (let i = 0; i < 3; i++) {
					...
					if (!bestMove || !bestDamage) break;
					this.add('-activate', source, 'ability: Nose Formation');
					this.damage(bestDamage, foe, source, bestMove);
				}
			}
		},
		onSourceAfterFaint(length, target, source, effect) {
	...
	royalcurrent: {
		...
		onSourceDamagingHit(damage, target, source, move) {
			if (move.crit) this.boost({ spe: 1 }, source, source);
		},
		flags: { breakable: 1 },
		name: "Royal Current",
		rating: 4,
	...
	wickedsnare: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, source, target, move) {
			if (target?.newlySwitched) return this.chainModify(1.5);
		},
		onAfterMoveSecondarySelf(source, target, move) {
	...
	auroraresonance: {
		...
		onTryHit(target, source, move) {
			if (target !== source && this.movehasType(move, 'Water')) {
				if (!this.heal(target.baseMaxhp / 4)) this.add('-immune', target, '[from] ability: Aurora Resonance');
				return null;
			}
		},
		onResidual(pokemon) {
	...
	sandsovereign: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, source, target, move) {
			if (source.effectiveWeather() === 'sandstorm' && move.category !== 'Status') return this.chainModify(1.2);
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (target.effectiveWeather() === 'sandstorm' && move.category !== 'Status') return this.chainModify(0.7);
		},
		onResidual(pokemon) {
			if (pokemon.effectiveWeather() !== 'sandstorm') return;
			for (const target of this.getAllActive()) {
				if (!target || target.fainted || target.hasType(['Rock', 'Ground', 'Steel'])) continue;
	...
	dunetyrant: {
		onStart(source) {
			this.field.setWeather('sandstorm', source, this.dex.abilities.get('dunetyrant'));
			if (this.field.isTerrain(['holyterrain', 'crystalcavernterrain', 'darkcrystalcavernterrain', 'newworldterrain', 'starlightarenaterrain', 'caveterrain', 'volcanicterrain', 'desertterrain'])) {
				this.boost({ def: 1, spd: 1 }, source, source, this.effect);
			}
		},
		...
		onModifyDef(def, pokemon) {
			if (pokemon.effectiveWeather() === 'sandstorm') return this.chainModify(1.5);
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (target.effectiveWeather() === 'sandstorm' && move.category !== 'Status') return this.chainModify(0.8);
		},
		flags: { breakable: 1 },
		name: "Dune Tyrant",
		rating: 4.5,
	...
	frostsovereign: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, source, target, move) {
			if (['hail', 'snow'].includes(source.effectiveWeather()) && move.category !== 'Status') return this.chainModify(1.2);
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (['hail', 'snow'].includes(target.effectiveWeather()) && move.category !== 'Status') return this.chainModify(0.7);
		},
		onResidual(pokemon) {
			if (!['hail', 'snow'].includes(pokemon.effectiveWeather())) return;
			for (const target of this.getAllActive()) {
				if (!target || target.fainted || target.hasType('Ice')) continue;
				this.damage(target.baseMaxhp / 16, target, pokemon);
			}
		},
		onDamagingHit(damage, target, source, move) {
			...
			this.add('-activate', target, 'ability: Frost Sovereign');
	...
	freezerburn: {
		onStart(source) {
			this.field.setWeather('hail', source, this.dex.abilities.get('freezerburn'));
		},
		onModifyTypePriority: -1,
		onModifyType(move, pokemon) {
			const noModifyType = [
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.typeChangerBoosted === this.effect || this.movehasType(move, 'Fire')) {
				return this.chainModify(1.2);
			}
		},
	...
	enlightenment: {
		...
		onModifyMove(move) {
			if (move.flags['contact']) move.ignoreAbility = true;
		},
		onAfterMoveSecondarySelf(source, target, move) {
			if (!target || target === source || source.isAlly(target) || target.fainted || move.category === 'Status') return;
	...
	relentlesslink: {
		onModifyMove(move) {
			if (move.multihit && Array.isArray(move.multihit)) move.multihit = move.multihit[1];
	...
	uncheckedassault: {
		...
		onTryAddVolatile(status, pokemon) {
			if (status.id === 'paralysis') return null;
		},
	...
	heavenlychorus: {
		...
		onSetStatus(status, target, source, effect) {
			return this.dex.abilities.get('naturalcure').onSetStatus?.call(this, status, target, source, effect);
		},
		onSwitchOut(pokemon) {
	...
	venombastion: {
		...
		onSourceModifyDamage(damage, source, target, move) {
			if (move.category !== 'Status' && this.checkMoveMakesContact(move, source, target)) return this.chainModify(0.75);
		},
		flags: { breakable: 1 },
		name: "Venom Bastion",
		rating: 4,
	...
	furnaceengine: {
		onTryHit(target, source, move) {
			if (target !== source && this.movehasType(move, ['Water', 'Fire'])) {
		...
		onResidual(pokemon) {
			for (const target of pokemon.foes()) {
				if (!target.hasType(['Fire', 'Rock', 'Ground']) && target.isGrounded()) {
					this.damage(target.baseMaxhp / 16, target, pokemon);
				}
			}
		},
	...
	deserttyrant: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, source, target, move) {
			if (source.effectiveWeather() === 'sandstorm' && this.movehasType(move, ['Ground', 'Rock'])) return this.chainModify(1.2);
		},
		onModifyDef(def, pokemon) {
			if (pokemon.hasType('Ground') && pokemon.effectiveWeather() === 'sandstorm') return this.chainModify(1.5);
		},
		onResidual(pokemon) {
			if (pokemon.effectiveWeather() !== 'sandstorm') return;
			for (const target of pokemon.foes()) {
				if (!target.hasType(['Ground', 'Rock', 'Steel']) && target.isGrounded()) {
					this.damage(target.baseMaxhp / 16, target, pokemon);
				}
			}
		},
	...
	heatcoil: {
		onResidual(pokemon) {
			if (pokemon.activeTurns) this.boost({ spe: 1 }, pokemon, pokemon);
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (move.flags['contact']) return this.chainModify(0.75);
		},
		onDamagingHit(damage, target, source, move) {
			if (source && source !== target && move.flags['contact']) this.damage(source.baseMaxhp / 16, source, target);
		},
		flags: { breakable: 1 },
		name: "Heat Coil",
		rating: 3.5,
	...
	ancientbloom: {
		...
		onStart(pokemon) {
			...
			this.heal(pokemon.baseMaxhp / 5, pokemon, pokemon);
			for (const ally of pokemon.adjacentAllies()) {
	...
	pollenbloom: {
		...
		onSourceModifyAtkPriority: 6,
		onSourceModifyAtk(atk, attacker, defender, move) {
			if (move && (this.movehasType(move, 'Ice') || this.movehasType(move, 'Fire'))) return this.chainModify(0.5);
		},
		onSourceModifySpAPriority: 5,
		onSourceModifySpA(atk, attacker, defender, move) {
			if (move && (this.movehasType(move, 'Ice') || this.movehasType(move, 'Fire'))) return this.chainModify(0.5);
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (this.movehasType(move, 'Grass')) return this.chainModify(1.3);
		},
		onResidual(pokemon) {
			for (const target of this.getAllActive()) {
	...
	waterbarrage: {
		...
		onImmunity(type, pokemon) {
			if (type === 'brn' || type === 'hail') return false;
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (this.movehasType(move, 'Water')) return this.chainModify(1.3);
		},
		onResidual(pokemon) {
	...
	royalarmament: {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (['Normal', 'Flying', 'Rock', 'Bug', 'Steel', 'Grass', 'Psychic', 'Ice', 'Dragon', 'Fairy', 'Poison'].includes(type)) {
				return typeMod - 1;
			}
		},
		flags: {},
	...
	verdantdrake: {
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (this.movehasType(move, ['Grass', 'Dragon'])) return this.chainModify(1.2);
		},
		onSourceModifyDamage(damage, source, target, move) {
	...
	astralwitchcraft: {
		...
		onAllyTryHitSide(target, source, move) {
			if (target.isAlly(source) || move.hasBounced || !move.flags['reflectable']) return;
			const newMove = this.dex.getActiveMove(move.id);
			newMove.hasBounced = true;
			newMove.pranksterBoosted = false;
			this.actions.useMove(newMove, this.effectState.target, { target: source });
	...
	blazingtempo: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (attacker.boosts.spe > 0 && this.movehasType(move, ['Fire', 'Fighting'])) return this.chainModify(1.2);
		},
		flags: {},
		name: "Blazing Tempo",
		rating: 4.5,
	...
	ragingcurrent: {
		onModifySpe(spe, pokemon) {
			if (pokemon.effectiveWeather() === 'raindance') return this.chainModify(2);
		},
		onTryHit(target, source, move) {
			if (move.id === 'explosion' || move.id === 'mindblown' || move.id === 'mistyexplosion' || move.id === 'selfdestruct') {
				this.add('-ability', target, 'Raging Current');
				return null;
			}
		},
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'brn') return;
			if ((effect as Move)?.status) this.add('-immune', target, '[from] ability: Raging Current');
			return false;
		},
		onSourceModifyAtkPriority: 6,
		onSourceModifyAtk(atk, attacker, defender, move) {
	...
	toxicbloom: {
		...
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (move.id === 'gigadrain') return this.chainModify(2);
		},
		onResidual(pokemon) {
			this.heal(pokemon.baseMaxhp / 16, pokemon, pokemon);
		},
		onTryHit(target, source, move) {
			if (target !== source && this.movehasType(move, 'Poison')) {
				if (!this.boost({ atk: 1, spa: 1 }, target, target)) {
					this.add('-immune', target, '[from] ability: Toxic Bloom');
				}
				return null;
			}
		},
		onAnyRedirectTarget(target, source, source2, move) {
			if (!this.movehasType(move, 'Poison')) return;
			const redirectTarget = ['randomNormal', 'adjacentFoe'].includes(move.target) ? 'normal' : move.target;
			if (this.validTarget(this.effectState.target, source, redirectTarget)) {
				if (move.smartTarget) move.smartTarget = false;
				if (this.effectState.target !== target) this.add('-activate', this.effectState.target, 'ability: Toxic Bloom');
				return this.effectState.target;
			}
		},
		flags: { breakable: 1 },
	...
	calderacore: {
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (this.movehasType(move, ['Fire', 'Ground'])) return this.chainModify(1.2);
		},
		onSourceModifyDamage(damage, source, target, move) {
			if (this.movehasType(move, 'Water')) return this.chainModify(0.5);
		},
		...
		onResidual(pokemon) {
			for (const target of pokemon.foes()) {
				if (!target.hasType(['Fire', 'Ground', 'Rock']) && target.isGrounded()) {
					this.damage(target.baseMaxhp / 16, target, pokemon);
				}
			}
		},
	...
	ironcognition: {
		...
		onTryBoost(boost, target, source, effect) {
			return this.dex.abilities.get('fullmetalbody').onTryBoost?.call(this, boost, target, source, effect);
		},
		onModifyDef(def, pokemon) {
			return this.dex.abilities.get('prismarmor').onModifyDef?.call(this, def, pokemon);
		},
