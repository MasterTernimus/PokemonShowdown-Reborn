...
export const Moves: { [moveid: string]: MoveData } = {
	...
	forestscurse: {
		...
		onHit(target) {
			...
			if (this.field.isTerrain('fairytaleterrain') || this.field.isTerrain('forestterrain')) {
	...
	grasspledge: {
		...
		flags: { protect: 1, mirror: 1, nonsky: 1, metronome: 1},
		onBasePower() {
			if (this.field.terrainState.Tchanges?.includes('waterpledge') || this.field.terrainState.Tchanges?.includes('firepledge')) {
				return this.chainModify([5325, 4096]);
			}
		},
		onAfterHit(target, source, move) {
			...
			onAfterMove(target, source, move) {
				...
				if (terrainChangeMoves.includes(move.id)) {
					if (this.field.terrainState.Tchanges?.includes('corrosivemistterrain') || move.id === 'aciddownpour') {
						...
						this.field.terrainState.Tchanges?.push('corrosivemistterrain')
					}
				}
		...
		onHit(pokemon) {
			...
			case 'snow':
				factor = 0.25;
					break;
			default:
					if (this.field.isTerrain('darkcrystalcavernterrain') || this.field.isTerrain('newworldterrain') || this.field.isTerrain('starlightarenaterrain')) {
					factor = 0.75;
				}
				break;
			}
