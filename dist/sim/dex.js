...
var import_dex_terrains = require("./dex-terrains");
/**
 * Dex
...
class ModdedDex {
  constructor(mod = "base") {
    ...
    this.Terrain = import_dex_terrains.DataTerrain;
    this.name = "[ModdedDex]";
    this.toID = Data.toID;
    this.gen = 0;
    this.parentMod = "";
    ...
    this.terrains = new import_dex_terrains.DexTerrains(this);
  ...
  getEffectiveness(source, target) {
    const sourceTypes = Array.isArray(source) ? source : typeof source === "string" ? [source] : [source.type];
    const targetTyping = target.getTypes?.() || target.types || (typeof target === "string" ? [target] : target);
    let totalTypeMod = 0;
  ...
  loadTextData() {
    ...
    dexes["base"].textCache = {
      ...
      Terrains: this.loadTextFile("terrains", "TerrainsText"),
