...
var import_conditions = require("./conditions");
var import_abilities = require("./abilities");
...
const Moves = {
  ...
  */
  ...
  reset: {
    ...
    onHit(pokemon) {
      ...
      if (pokemon.species.baseSpecies === "Aegislash") {
        ...
        (0, import_abilities.changeSet)(this, pokemon, import_random_teams.ssbSets[specialSet ? "aegii" : "aegii-Alt"]);
  ...
  lookingcool: {
    ...
    onTryMove(target) {
      if (target.volatiles["lookingcool"])
        return false;
      this.attrLastMove("[still]");
    },
    onPrepareHit(target, source) {
  ...
  lilypadoverflow: {
    ...
    basePowerCallback(source, target, move) {
      if (!source.volatiles["raindrop"]?.layers)
    ...
    onAfterMove(pokemon) {
      if (pokemon.volatiles["raindrop"])
        pokemon.removeVolatile("raindrop");
    },
    secondary: null,
    target: "normal",
    type: "Water"
  ...
  ravelin: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      ...
      if (pokemon.side.getSideCondition("lightscreen"))
        return;
  ...
  datacorruption: {
    ...
    onHit(target, source) {
      ...
      const newMoveSlots = (0, import_abilities.changeMoves)(this, target, newMoves);
  ...
  broadsidebarrage: {
    ...
    onPrepareHit(target, source) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Archas")}|Fire all guns! Fiiiiire!`);
    },
  ...
  radiantburst: {
    ...
    onHit(target, pokemon) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Arcticblast")}|YEET`);
      if (pokemon.volatiles["brilliant"])
        pokemon.removeVolatile("brilliant");
    },
    secondary: null,
    target: "normal",
    type: "Fairy"
  ...
  baneterrain: {
    ...
    condition: {
      ...
      onModifyMove(move, source, target) {
        if (move.overrideOffensiveStat && !["atk", "spa"].includes(move.overrideOffensiveStat))
          return;
    ...
    condition: {
      onStart(target) {
        ...
        for (const moveSlot of target.moveSlots) {
          if (!(moveSlot.id === "swordsdance" || moveSlot.id === "pandorasbox"))
            continue;
      ...
      onEnd(pokemon) {
        if (!pokemon.m.backupMoves)
          return;
        for (const [index, moveSlot] of pokemon.moveSlots.entries()) {
          if (!pokemon.m.replacedMoves.includes(moveSlot.move))
            continue;
  ...
  kevin: {
    ...
    onTryImmunity(target, pokemon) {
      if (pokemon.types[1]) {
        if (!target.runImmunity(pokemon.types[1]))
          return false;
      }
      return true;
    },
  ...
  getoffmylawn: {
    ...
    onHit() {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("drampa's grandpa")}|GET OFF MY LAWN!!!`);
    },
    secondary: null,
    forceSwitch: true,
    target: "normal",
  ...
  cloakdance: {
    ...
    onHit(target, source) {
      if (!source.abilityState.busted || source.side.pokemonLeft === 1) {
        ...
        if (source.species.baseSpecies === "Mimikyu")
  ...
  youhavenohope: {
    ...
    onHit(target, source) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("explodingdaisies")}|You have no hope ${target.name}!`);
    },
    secondary: null,
    target: "normal",
    type: "Normal"
  ...
  soupstealing7starstrikeredux: {
    ...
    onHit(target, source) {
      ...
      if (this.randomChance(1, 2)) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("fart")}|I hl on soup`);
      } else {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("fart")}|I walk with purpose. bring me soup.`);
      }
    },
    condition: {
      duration: 2,
  ...
  cradilychaos: {
    ...
    onHit(target, source, move) {
      ...
      if (source.species.baseSpecies === "Alcremie") {
        const formes = ["Finland", "Finland-Tsikhe", "Finland-Nezavisa", "Finland-J\xE4rvilaulu"].filter((forme) => import_random_teams.ssbSets[forme].species !== source.species.name);
        const newSet = this.sample(formes);
        (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets[newSet]);
      }
    },
    secondary: null,
    target: "normal",
  ...
  randomscreaming: {
    ...
    secondary: {
      ...
      status: "frz",
      onHit() {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Gimmick")}|Show me some more paaain, baaaby`);
      }
    },
    thawsTarget: true,
    target: "normal",
  ...
  gacha: {
    ...
    onHit(target, source, move) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("GMars")}|${message}`);
  ...
  landfall: {
    ...
    sideCondition: "stormsurge",
    // Programmed in conditions.ts
    target: "normal",
    type: "Water"
  },
  // Hubriz
  ...
  thehuntison: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (!target || target.fainted || target.hp <= 0) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Jett")}|Owned!`);
      }
    },
    condition: {
      duration: 1,
    ...
    onHit(pokemon) {
      if (pokemon.species.baseSpecies === "Toxtricity") {
        if (pokemon.species.forme === "Low-Key") {
          (0, import_abilities.changeSet)(this, pokemon, import_random_teams.ssbSets["Jho"]);
        } else {
          (0, import_abilities.changeSet)(this, pokemon, import_random_teams.ssbSets["Jho-Low-Key"]);
        }
      }
    },
    boosts: {
  ...
  cozycuddle: {
    ...
    onTryHit(target, source, move) {
      if (target.volatiles["cozycuddle"])
        return false;
  ...
  leaveittotheteam: {
    ...
    condition: {
      ...
      onSwitchIn(target) {
        const positions = this.effectState.positions;
  ...
  kipup: {
    ...
    condition: {
      duration: 1,
      onStart(pokemon) {
        this.add("-message", "This Pok\xE9mon prepares itself to be knocked down!");
      },
      onDamagePriority: -10,
      onDamage(damage, target, source, effect) {
        if (this.effectState.gotHit)
          return damage;
      ...
      onHit(pokemon, source, move) {
        ...
        if (!pokemon.isAlly(source) && move.category !== "Status") {
          ...
          const boosts = { def: 2, spd: 2 };
          if (pokemon.boosts.def >= 1)
            boosts.def--;
          if (pokemon.boosts.spd >= 1)
            boosts.spd--;
      ...
      if (source.species.id.includes("unown")) {
        const monList = Object.keys(this.dex.data.Pokedex).filter((speciesid) => {
          ...
          if (species.id.startsWith("unown"))
            return false;
          if (species.isNonstandard && ["Gigantamax", "Unobtainable"].includes(species.isNonstandard))
            return false;
          if (["Arceus", "Silvally"].includes(species.baseSpecies) && species.types[0] !== "Normal")
            return false;
          if (species.baseStats.spa < 80)
            return false;
          if (species.baseStats.spe < 80)
            return false;
          const unownLetter = source.species.id.charAt(5) || "a";
          if (!species.id.startsWith(unownLetter.trim().toLowerCase()))
            return false;
          return true;
        });
  ...
  spamguess: {
    ...
    onHit(target, source) {
      for (const move of ["Haze", "Worry Seed", "Poison Powder", "Stun Spore", "Leech Seed"]) {
        ...
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Meicoo")}|That is not the answer - try again!`);
      }
  ...
  technotubertransmission: {
    ...
    onHit() {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Notater517")}|/html For more phantasmic music, check out <a href = "http://spo.ink/tunes">this link.</a>`);
    },
    self: {
  ...
  omzoom: {
    ...
    onHit() {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("OM~!")}|Bang Bang`);
    },
    flags: { protect: 1, mirror: 1 },
    selfSwitch: true,
    secondary: null,
  ...
  balefulblaze: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (!target || target.fainted || target.hp <= 0) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("PartMan")}|FOR SNOM!`);
  ...
  trickery: {
    ...
    onHit(target, source, effect) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Perish Song")}|/html <img src="https://i.imgflip.com/3rt1d8.png" width="262" height="150" />`);
      const item = target.takeItem(source);
      if (!target.item) {
        if (item)
          this.add("-enditem", target, item.name, "[from] move: Trickery", "[of] " + source);
  ...
  ghostof1v1past: {
    ...
    condition: {
      onAnyFaint(target) {
        if (target === this.effectState.targetPokemon)
          this.effectState.source.faint();
      }
    },
    secondary: null,
    target: "normal",
  ...
  dungeonsdragons: {
    ...
    onHit(target, source, move) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("PiraTe Princess")}|did someone say d&d?`);
      target.addVolatile("trapped", source, move, "trapper");
  ...
  croak: {
    ...
    onPrepareHit(target, source, move) {
      ...
      if (source.positiveBoosts() < 8) {
        const stats = [];
        let stat;
        const exclude = ["accuracy", "evasion"];
  ...
  shockedlapras: {
    ...
    onPrepareHit(target, source) {
      ...
      if (!source.status)
        this.add("-anim", source, "Thunder Wave", source);
    },
    onHit() {
  ...
  stonykibbles: {
    ...
    onHit() {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Raihan Kibana")}|Let the winds blow! Stream forward, Sandstorm!`);
    },
    onPrepareHit(target, source) {
      this.add("-anim", source, "Rock Slide", target);
  ...
  pitchblackterrain: {
    ...
    condition: {
      ...
      onHit(target, source, move) {
        if (!target.hp || target.species.name !== "Mismagius")
          return;
  ...
  homunculussvanity: {
    ...
    self: {
      onHit(source) {
        ...
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Sectonia")}|Jelly baby ;w;`);
      }
    },
    secondary: null,
    target: "self",
  ...
  tsukuyomi: {
    ...
    onHit(pokemon, source, move) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Segmr")}|I don't like naruto actually let someone else write this message plz.`);
  ...
  badopinion: {
    ...
    onHit() {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("sejesensei")}|Please go read To Love-Ru I swear its really good, wait... don\u2019t leave\u2026`);
    },
    self: {
      boosts: {
        def: 1
  ...
  legendaryswordsman: {
    ...
    onTry(source, target) {
      ...
      if (!move || move.category === "Status" && move.id !== "mefirst" || target.volatiles["mustrecharge"]) {
        if (move?.category === "Status") {
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Seso")}|Irritating a better swordsman than yourself is always a good way to end up dead.`);
        } else {
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Seso")}|Scars on the back are a swordsman's shame.`);
        }
        return false;
      }
    },
    ...
    onPrepareHit(target, source) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Seso")}|FORWARD!`);
  ...
  updraft: {
    ...
    condition: {
      ...
      onStart(target) {
        this.effectState.origTypes = target.getTypes();
        if (target.getTypes().length === 1) {
          if (!target.addType("Flying"))
            return false;
          this.add("-start", target, "typeadd", "Flying", "[from] move: Updraft");
        } else {
          const primary = target.getTypes()[0];
          if (!target.setType([primary, "Flying"]))
            return false;
          this.add("-start", target, "typechange", primary + "/Flying", "[from] move: Updraft");
        }
      },
      onEnd(target) {
        if (!target.setType(this.effectState.origTypes))
          return false;
    ...
    secondary: {
      chance: 100,
      onHit(target) {
        if (target.hasType(["Flying", "Ground"]) || target.volatiles["ingrain"] || target.volatiles["brilliant"])
          return false;
  ...
  veto: {
    ...
    onHit(target, source) {
      if (source.m.statsRaisedLastTurn) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Struchni")}|**veto**`);
      }
    },
    target: "normal",
    type: "Steel"
  ...
  rightoncue: {
    ...
    onHit(target, source) {
      ...
      if (successes === 1) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("tiki")}|truly a dumpster fire`);
      } else if (successes >= 4) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("tiki")}|whos ${source.side.foe.name}?`);
      }
    },
    secondary: null,
    target: "self",
  ...
  glitchexploiting: {
    ...
    onHit(target, source, move) {
      ...
      if (random === 1) {
        ...
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Volco")}|haha memory corruption go brrr...`);
    ...
    secondaries: [
      {
        ...
        onHit(target, source) {
          ...
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Volco")}|Ever just screw up the trick and corrupt the memory and cause the wrong thing to happen possibly ruining a run? No? Just me? okay...`);
          if (this.randomChance(1, 2)) {
  ...
  classchange: {
    ...
    onPrepareHit(foe, source, move) {
      const formes = ["Cleric", "Ninja", "Dancer", "Songstress", "Jester"].filter((forme) => import_random_teams.ssbSets[`yuki-${forme}`].species !== source.species.name);
    ...
    onHit(target, source) {
      if (source.baseSpecies.baseSpecies !== "Pikachu")
        return;
      switch (source.m.yukiCosplayForme) {
        case "Cleric":
          (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["yuki-Cleric"]);
          this.add("-message", "yuki patches up her wounds!");
          return;
        case "Ninja":
          (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["yuki-Ninja"]);
          this.add("-message", `yuki's fast movements confuse ${target.name}!`);
          return;
        case "Dancer":
          (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["yuki-Dancer"]);
          this.add("-message", `yuki dazzles ${target.name} with her moves!`);
          return;
        case "Songstress":
          (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["yuki-Songstress"]);
          this.add("-message", `yuki sang an entrancing melody!`);
          return;
        case "Jester":
          (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["yuki-Jester"]);
  ...
  ingredientforaging: {
    ...
    onAfterHit(target, source) {
      ...
      if (source.hp && target.takeItem(source)) {
        ...
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Zalm")}|Yum`);
  ...
  relicdance: {
    ...
    onAfterMove(source) {
      ...
      if (source.species.baseSpecies !== "Meloetta")
        return;
      if (source.species.name === "Meloetta-Pirouette") {
        (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["Zarel"]);
      } else {
        (0, import_abilities.changeSet)(this, source, import_random_teams.ssbSets["Zarel-Pirouette"]);
      }
    },
    onModifyMove(move, pokemon) {
      if (pokemon.species.name === "Meloetta-Pirouette")
        move.type = "Fighting";
    },
    target: "allAdjacentFoes",
    type: "Psychic"
  ...
  bigstormcoming: {
    ...
    onPrepareHit() {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("Zodiax")}|There is a hail no storm okayyyyyy`);
    },
    onTry(pokemon, target) {
  ...
  auroraveil: {
    ...
    onTryHitSide() {
      if (!this.field.isWeather(["winterhail", "heavyhailstorm", "hail"]))
        return false;
    }
  },
  blizzard: {
  ...
  dig: {
    ...
    condition: {
      ...
      onImmunity(type, pokemon) {
        if (["sandstorm", "winterhail", "heavyhailstorm", "hail"].includes(type))
          return false;
      },
    ...
    condition: {
      ...
      onModifySpD(spd, pokemon) {
        for (const target of this.getAllActive()) {
          if (target === pokemon)
            continue;
          if (target.hasAbility("conditionoverride"))
    ...
    onHit(target, source, effect) {
      ...
      for (const id in this.dex.data.Moves) {
        ...
        if (move.gen > this.gen)
          continue;
        if (move.isMax === true && move.basePower === 10)
          continue;
