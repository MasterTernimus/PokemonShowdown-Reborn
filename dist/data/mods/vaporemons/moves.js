...
const Moves = {
  ...
  healingstones: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.hasAbility("overcoat") || pokemon.hasItem("mantisclaw"))
          return;
        const healAmounts = [0, 3];
    ...
    condition: {
      onSwap(target) {
        if (!target.fainted) {
          ...
          if (damage)
            this.add("-heal", target, target.getHealth, "[from] move: Life Dew", "[of] " + source);
  ...
  hazardouswaste: {
    ...
    basePowerCallback(pokemon, target, move) {
      ...
      if (yourSide.getSideCondition("stealthrock"))
        allLayers++;
      if (yourSide.getSideCondition("healingstones"))
        allLayers++;
      ...
      if (targetSide.getSideCondition("stealthrock"))
        allLayers++;
      if (targetSide.getSideCondition("healingstones"))
        allLayers++;
  ...
  parry: {
    ...
    condition: {
      ...
      onFoeTryMove(target, source, move) {
        ...
        if ((source.isAlly(parryHolder) || move.target === "all") && (!source.hasAbility("innerfocus") || !source.hasAbility("shielddust") || !source.hasAbility("steadfast") || !source.hasItem("covertcloak") || !source.hasAbility("sandveil") && !this.field.isWeather("sandstorm") || !source.hasAbility("sunblock") && !this.field.isWeather("sunnyday") || !source.hasAbility("snowcloak") && !this.field.isWeather("snow")) && move.priority > 0.1) {
          this.attrLastMove("[still]");
  ...
  rebuild: {
    ...
    basePower: 0,
    category: "Status",
    shortDesc: "Restores HP equal to the user's level \xD7 1.25.",
    name: "Rebuild",
    pp: 10,
    priority: 0,
    flags: { snatch: 1, heal: 1, metronome: 1 },
  ...
  stickyweb: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded() || pokemon.hasItem("heavydutyboots") || pokemon.hasAbility("overcoat") || pokemon.hasItem("mantisclaw"))
          return;
  ...
  doubleshock: {
    ...
    self: {
      onHit(pokemon) {
        if (pokemon.hasItem("terashard") && pokemon.teraType === "Electric")
          return;
  ...
  burnup: {
    ...
    self: {
      onHit(pokemon) {
        if (pokemon.hasItem("terashard") && pokemon.teraType === "Fire")
          return;
